...
| **백지 공포증** | 설치만 하고 시작 못함   | 첫 발 막막함 | 도구 탓으로 돌림 |
...
### 안티 패턴 4: 백지 공포증 (Blank Canvas Syndrome)

**증상**: 옵시디언을 설치하고 볼트를 만들었지만, 텅 빈 화면 앞에서 멈춰버림. "뭐부터 해야 하지?"라는 막막함에 결국 앱을 닫고, 다시 열지 않게 됨.

> "옵시디언 열심히 쓰시는 거 보고 나도 한번 해보고 싶다 해서 깔긴 했는데... 처음 그 백지 화면 그대로예요." - 실제 사용자 인터뷰

**원인**:
- 템플릿 볼트가 이미 구조화되어 있어 오히려 부담
- "첫 발을 어디에 디딜지"에 대한 가이드 부재
- 다른 사람의 완성된 시스템을 보면 더 위축됨
- 완벽주의와 결합되면 시작 자체를 무한 연기

**업무 사례**:
K씨는 회사에서 지식 관리의 필요성을 느끼고 옵시디언을 설치했다. 하지만 폴더 구조(Ingest, AI, Topics 등)를 보니 어디서 시작해야 할지 모르겠다. 첫 노트를 뭘로 쓸지 고민하다가 "나중에 시간 날 때 제대로 해야지"하며 닫아버린다. 그 "나중"은 오지 않는다.

**다이어트 사례**:
L씨는 건강 관리 앱을 설치하고 처음 켰을 때 수십 개의 빈 카테고리를 봤다. 체중, 식단, 운동, 수면, 물 섭취... "다 채워야 하나?" 부담감에 그냥 앱을 삭제한다.

**예방 핵심**: 첫 번째 단계는 **인제스트(Ingest)**다. 일단 외부에 있는 뭔가를 넣어서 백지 상태를 탈출하자.

> [!tip] 백지 탈출 작전
> 1. 오늘 읽은 기사 하나를 클리핑한다 (5분)
> 2. 오늘 있었던 일 한 줄을 쓴다 (1분)
> 3. 끝. 내일 다시 하나 더.
>
> 완벽하지 않아도 된다. **일단 시작하는 것**이 핵심이다.
